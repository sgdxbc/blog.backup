...
在[上一篇文章]({% post_url 2019-12-24-how-to-optimize-optimizing %})中我揭示了基本块优化的最终目的，和（有可能）解决其内存消耗过高的思路——将一些条件分支保持为「折叠」状态，而不是成为基本块的分割线。为了便于说明，我将必须被展开融入基本块数据结构的条件分支称为`Choice`，而可以作为普通的指令滥竽充数的条件分支称为`If`。
